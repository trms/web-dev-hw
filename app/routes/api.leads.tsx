...
async function handleJsonBody(jsonData: any) {
  const lead: Lead = {
    ...
    yearlyCaptionMins:
      jsonData.averageProgramsPerMonth *
      jsonData.averageLengthOfProgramsInHours *
      60,
  };
  saveLead(lead);
...
async function handleFormData(formData: FormData) {
  ...
  const averageProgramsPerMonth =
    Number(formData.get('averageProgramsPerMonth')) ?? 0;
  const averageLengthOfProgramsInHours =
    Number(formData.get('averageLengthOfProgramsInHours')) ?? 0;
  const needsTranslations = formData.get('needsTranslations') == 'YES';
  const yearlyCaptionMins =
    averageProgramsPerMonth * averageLengthOfProgramsInHours * 60;
  const lead: Lead = {
    name,
    email,
